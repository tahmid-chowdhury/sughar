...
import { SpecificServiceRequestContactsPage } from './SpecificServiceRequestContactsPage';
...
const ChatBubble: React.FC<{ message: ChatMessage; currentUserName?: string }> = ({ message, currentUserName }) => {
...
}> = ({requestData, requester, building, currentUser}) => (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <Card>
                <div className="flex items-center mb-6">
                    ...
                    <div className="ml-4">
                        ...
                        <span className="ml-2 px-2 py-0.5 text-xs font-semibold text-pink-800 bg-pink-100 rounded-full">{requester.rating}</span>
                    </div>
                </div>
        ...
        <div className="lg:col-span-1">
            <Card className="flex flex-col h-full">
                ...
                <div className="flex-1 space-y-4 overflow-y-auto pr-2 bg-gray-50 p-4 rounded-lg max-h-64">
                   {/* Sample comments/updates */}
                   <div className="bg-white p-3 rounded-lg shadow-sm">
                       <div className="flex items-start gap-2">
                           <img src={requester.avatar} alt={requester.name} className="w-8 h-8 rounded-full" />
                           <div className="flex-1">
                               <p className="text-xs text-gray-500">{requester.name} • {requestData.requestDate}</p>
                               <p className="text-sm text-gray-700 mt-1">Request submitted and under review.</p>
                           </div>
                       </div>
                   </div>
                   {requestData.assignedContact && (
                       <div className="bg-white p-3 rounded-lg shadow-sm">
                           <div className="flex items-start gap-2">
                               <img src={requestData.assignedContact.avatar} alt={requestData.assignedContact.name} className="w-8 h-8 rounded-full" />
                               <div className="flex-1">
                                   <p className="text-xs text-gray-500">{requestData.assignedContact.name} • System</p>
                                   <p className="text-sm text-gray-700 mt-1">Contractor assigned to this request.</p>
                               </div>
                           </div>
                       </div>
                   )}
...
export const SpecificServiceRequestPage: React.FC<SpecificServiceRequestPageProps> = ({ serviceRequestId, appData, onBack, currentUser }) => {
    ...
    const renderContent = () => {
        switch (activeTab) {
            case 'Images':
                // Mock images for the service request
                const mockImages = [
                    { id: '1', url: 'https://picsum.photos/seed/sr1/400/300', type: 'image' as const, uploadedBy: requester.name, uploadDate: requestData.requestDate },
                    { id: '2', url: 'https://picsum.photos/seed/sr2/400/300', type: 'image' as const, uploadedBy: requester.name, uploadDate: requestData.requestDate },
                ];
                return <SpecificServiceRequestImagesPage media={mockImages} />;
            case 'Activity Log':
                return (
            ...
            case 'Contacts':
                // For tenants: hide contractor ads, show simplified message
                if (isTenant) {
                    return (
                        <div className="text-center py-12">
                            <p className="text-text-secondary">Contact information will be displayed here when contractors are assigned.</p>
                        </div>
                    );
                }
                // For landlords: show contractor information with ads
                // The DidYouKnowCard and SuggestedVendorCards are only shown to landlords
                const contactCards = [
                    {
                ...
                const suggestedVendors = [
                    { imageUrl: 'https://picsum.photos/seed/vendor1/200', logo: 'star' as const, name: 'Premium Plumbing', rating: 4.9 },
                    { imageUrl: 'https://picsum.photos/seed/vendor2/200', logo: 'star' as const, name: 'Quick Fix Services', rating: 4.8 },
                ];
                return <SpecificServiceRequestContactsPage contactCards={contactCards} suggestedVendors={suggestedVendors} />;
            case 'Summary':
            default:
                return <ServiceRequestSummary requestData={requestData} requester={requester} building={building} currentUser={currentUser} />;
        }
    };
