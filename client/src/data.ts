...
const generateInitialData = (): AppData => {
    ...
    const tenants: Tenant[] = tenantsData.map((t, i) => ({
        id: `T-${i+1}`,
        name: t.name,
        avatar: `https://i.pravatar.cc/40?u=${t.name}`,
        building: '', // Will be populated later
        unit: '', // Will be populated later
        leaseProgress: Math.floor(Math.random() * 80) + 10,
        rentStatus: i % 10 === 0 ? RentStatus.Overdue : RentStatus.Paid,
        requests: 0, // Will be populated later
    ...
    for (let i = 0; i < 28; i++) {
        ...
        units.push({
            ...
            monthlyRent: 20000 + Math.floor(Math.random() * 15000),
            status: status,
    ...
    const serviceRequests: ServiceRequest[] = Object.entries(srDetails).map(([id, details]) => {
        ...
        return {
            ...
            completionDate: id === 'SR-0002' ? '2025-09-28' : undefined,
        }
    });
    ...
    units.forEach((unit, i) => {
        if(unit.status === UnitStatus.Rented) {
            documents.push({
                id: `DOC-L-${i}`, name: `Lease - ${unit.unitNumber}, ${unit.buildingId}`, type: DocumentType.Lease, uploadDate: new Date(new Date().setMonth(new Date().getMonth() - i)).toISOString().split('T')[0], building: unit.buildingId, unit: unit.unitNumber, amount: unit.monthlyRent, category: 'Income'
            });
        }
    });
    documents.push({
        id: 'DOC-E-1', name: 'Plumbing Repair Invoice', type: DocumentType.Service, uploadDate: '2024-09-01', building: 'B-LC', unit: '4A', amount: 5000, category: 'Expense'
    });
     // Add some income docs for current month for the home page card
    documents.push({id: 'DOC-INC-CUR-1', name: 'Rent Payment', type: DocumentType.Income, uploadDate: new Date().toISOString().split('T')[0], building: 'B-BH', unit: '1A', amount: 30000, category: 'Income'});
    documents.push({id: 'DOC-INC-CUR-2', name: 'Rent Payment', type: DocumentType.Income, uploadDate: new Date().toISOString().split('T')[0], building: 'B-DR', unit: '2A', amount: 25000, category: 'Income'});
